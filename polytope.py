...
class Main(ttk.Frame):
    ...
    def _initUI(self):
        ...
        _xwRotBtn = ttk.Button(_guiRight, text="xw", width=5,
                               command=lambda: self.canvas.set_rotax('xw'))
        _xwRotBtn.bind('<Key-Return>',lambda event:self.canvas.set_rotax('xw'))
        _xwRotBtn.grid(row=2, column=0)
        _ywRotBtn = ttk.Button(_guiRight, text="yw", width=5,
                               command=lambda: self.canvas.set_rotax('yw'))
        _ywRotBtn.bind('<Key-Return>',lambda event:self.canvas.set_rotax('yw'))
        _ywRotBtn.grid(row=2, column=1)
        _zwRotBtn = ttk.Button(_guiRight, text="zw", width=5,
                               command=lambda: self.canvas.set_rotax('zw'))
        _zwRotBtn.bind('<Key-Return>',lambda event:self.canvas.set_rotax('zw'))
        _zwRotBtn.grid(row=2, column=2)
        _xyRotBtn = ttk.Button(_guiRight, text="xy", width=5,
                               command=lambda: self.canvas.set_rotax('xy'))
        _xyRotBtn.bind('<Key-Return>',lambda event:self.canvas.set_rotax('xy'))
        _xyRotBtn.grid(row=3, column=0)
        _yzRotBtn = ttk.Button(_guiRight, text="yz", width=5,
                               command=lambda: self.canvas.set_rotax('yz'))
        _yzRotBtn.bind('<Key-Return>',lambda event:self.canvas.set_rotax('yz'))
        _yzRotBtn.grid(row=3, column=1)
        _xzRotBtn = ttk.Button(_guiRight, text="xz", width=5,
                               command=lambda: self.canvas.set_rotax('xz'))
        _xzRotBtn.bind('<Key-Return>',lambda event:self.canvas.set_rotax('xz'))
        ...
        _aBarBtn = ttk.Button(_guiRight, text="(pqs)", width=5,
                              command=lambda: self.canvas.set_select('a'))
        _aBarBtn.bind('<Key-Return>', lambda event:self.canvas.set_select('a'))
        _aBarBtn.grid(row=5, column=0)
        self.bBrBn = ttk.Button(_guiRight, text="(|pqs)", width=5,
                              command=lambda: self.canvas.set_select('b'))
        self.bBrBn.bind('<Key-Return>',lambda event:self.canvas.set_select('b'))
        self.bBrBn.grid(row=5, column=1)
        _cBarBtn = ttk.Button(_guiRight, text="(pqs|)", width=5,
                              command=lambda: self.canvas.set_select('c'))
        _cBarBtn.bind('<Key-Return>', lambda event:self.canvas.set_select('c'))
        _cBarBtn.grid(row=5, column=2)
        _pBarBtn = ttk.Button(_guiRight, text="(p|qs)", width=5,
                              command=lambda: self.canvas.set_select('p'))
        _pBarBtn.bind('<Key-Return>', lambda event:self.canvas.set_select('p'))
        _pBarBtn.grid(row=6, column=0)
        _qBarBtn = ttk.Button(_guiRight, text="(q|sp)", width=5,
                              command=lambda: self.canvas.set_select('q'))
        _qBarBtn.bind('<Key-Return>', lambda event:self.canvas.set_select('q'))
        _qBarBtn.grid(row=6, column=1)
        _sBarBtn = ttk.Button(_guiRight, text="(s|pq)", width=5,
                              command=lambda: self.canvas.set_select('s'))
        _sBarBtn.bind('<Key-Return>', lambda event:self.canvas.set_select('s'))
        _sBarBtn.grid(row=6, column=2)
        _pqBarBtn = ttk.Button(_guiRight, text="(pq|s)", width=5,
                               command=lambda: self.canvas.set_select('pq'))
        _pqBarBtn.bind('<Key-Return>',lambda event:self.canvas.set_select('pq'))
        _pqBarBtn.grid(row=7, column=0)
        _qsBarBtn = ttk.Button(_guiRight, text="(qs|p)", width=5,
                               command=lambda: self.canvas.set_select('qs'))
        _qsBarBtn.bind('<Key-Return>',lambda event:self.canvas.set_select('qs'))
        _qsBarBtn.grid(row=7, column=1)
        _spBarBtn = ttk.Button(_guiRight, text="(sp|q)", width=5,
                               command=lambda: self.canvas.set_select('sp'))
        _spBarBtn.bind('<Key-Return>',lambda event:self.canvas.set_select('sp'))
        ...
        self.unitDist = 20
        self.distance.set(int(ZOOM*RADIUS*RETINA/(self.unitDist**(3/2))))
    ...
    def change(self, change, value=0):
        ...
        if change == 's':
            ...
            self.bBrBn.config(state=tk.DISABLED)
        elif change == 'wire':
            self.wire.set(1)
            ...
            try:                        # Canvas initializes before zoom
                self.zoom.set(ZOOM)
                self.unitDist = 20  # Distance follows a x^(-3/2) curve
                self.distance.set(int(ZOOM*RADIUS*RETINA/self.unitDist**(3/2)))
                self.bBrBn.config(state=tk.NORMAL)
                self.wireCheck.config(state=tk.NORMAL)
            except:
                return
        self.canvas.render()
...
class Canvas(tk.Canvas):
    ...
    def set_rotax(self, rotAxis):
        ...
        self._currPolytope.set_rotax(self.rotAxis)
        self._sphere.set_rotax(self.rotAxis)
        self.render()
        if self._currPolytope.get_points():
    ...
    def _schlafli3D(self, entry):
        ...
        # Take 3D Schlafli symbol, return its spherical coordinates and edges.
    ...
    def _wythoff(self, entry='', selection='a', colour='#000'):
        ...
        if sorted(self._currWythoff) not in SNUBABLE:
            if selection == 'b':
                raise ValueError
            else:
                self._noSnub = 1
