"""
Polyhedron Shader v0.75

This program shades convex polyhedra.
All the faces of a single type can now have the same random colour.
This program finally works, though _wythoff_snub is now disabled in most cases.
Problems that remain with the shader are that many faces share a side at times,
that _set_faces renders all polyhedra with the same vertices and edges the same,
and that polyhedra composed only of 'star' triangles are not recognized as star.
"""
...
class Main(ttk.Frame):
    ...
    def _initUI(self):
        ...
        self.bBarBtn = ttk.Button(_guiRight, text="(|pqs)", width=5,
                              command=lambda: self.canvas.set_selection('b'))
        self.bBarBtn.bind('<Key-Return>', lambda event:self.canvas.set_selection('b'))
        self.bBarBtn.grid(row=5, column=1)
    ...
    def change(self, change, value=0):
        ...
        if change == 's':
            ...
            self.bBarBtn.config(state=tk.DISABLED)
        elif change == 'wire':
            self.wire.set(1)
            ...
            try:                        # Canvas initializes before zoom
                ...
                self.distance.set(int(ZOOM*RADIUS*RETINA/(self.unitDistance**(3/2))))
                self.bBarBtn.config(state=tk.NORMAL)
                self.wireCheck.config(state=tk.NORMAL)
            except:
                return
        self.canvas.render()
