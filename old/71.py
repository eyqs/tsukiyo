"""
Polyhedron Shader v0.71

This program shades convex polyhedra.
The viewing axis, rotation axis, distance, and zoom can now be
modified within inputText by prefixing v, r, d, and z respectively.
Canvas now stores the rotAxis, but rotation is still limited to six planes.
"""
...
class Canvas(tk.Canvas):
            ...
            try:
                ...
                if len(rotAxis) == 2:           # 3D rotation plane, two vectors
                    ...
                    if (len(u) != 4 or len(v) != 4
                        or sum(u) != 1 or sum(v) != 1):     # Only 1 works now
                        return ValueError
                    for i in range(4):
                        if ((u[i] > 0 and u[i] < 1) or
                            (v[i] > 0 and v[i] < 1)):
                            return ValueError
    ...
    def _reset(self):
        ...
        if self.get_data('star') == 1 and self.parent.wire.get() == 0:
...
class Polytope():
    ...
    def set_rotaxis(self, axes):
        """Set the axes of rotation of the polytope."""
        self._axis_i = axes[0]
        self._axis_j = axes[1]
