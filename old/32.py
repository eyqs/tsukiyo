"""
Polychoron Visualizer v0.32

This script displays the regular polyhedra.
The interface and internal coordinate system now work for polychora.
"""
...
TITLE = "Polychoron Visualizer v0.32"
DESCRIPTION = "\nThis script displays the regular polyhedra."
pi = math.pi
WIDTH = 470
HEIGHT = 450
...
RADIUS = 100
DISTANCE = 5
CHANGE = 5    # 5 pixels change in radius and 0.1r change in distance
...
class Main(ttk.Frame):
    ...
    def _initUI(self):
        ...
        _radiusLabel = ttk.Label(_guiFrame, text="r: ")
        _radiusLabel.grid(row=3, column=1, sticky=tk.E)
        _rUpBtn = ttk.Button(_guiFrame, image=self._upBtn, command=lambda:
                             self.canvas.change('rUp'))
        _rUpBtn.bind("<Button-1>", lambda event: self._mouse_down('rUp'))
        _rUpBtn.bind("<ButtonRelease-1>", self._mouse_up)
        _rUpBtn.bind("<Key-Return>", lambda event:
                     self.canvas.change('rUp'))
        _rUpBtn.grid(row=3, column=2)
        _rDownBtn = ttk.Button(_guiFrame, image=self._downBtn, command=lambda:
                               self.canvas.change('rDown'))
        _rDownBtn.bind("<Button-1>", lambda event: self._mouse_down('rDown'))
        _rDownBtn.bind("<ButtonRelease-1>", self._mouse_up)
        _rDownBtn.bind("<Key-Return>", lambda event:
                       self.canvas.change('rDown'))
        _rDownBtn.grid(row=3, column=3)
...
class Canvas(tk.Canvas):
    ...
    def _schlafli3D(self, entry):
        ...
        if round(firstPhi,4) <= round(pi/2,4):    # Rounding for float errors
            if round(firstPhi,4) < round(pi/2,4):
