...
There are no duplicate points in these reflections
but the old method remains and edges are temporary.
"""
...
TITLE = "Wythoff Visualizer v0.48"
    ...
    def _wythoff(self, entry):
        ...
        while depth > 0:
            for triangle in reversed(triangles):   # Reversed to avoid loop
                ...
                kq = sum([oq[t]*ps[t] for t in range(3)])
                ks = sum([os[t]*pq[t] for t in range(3)])
                knp = sum([on[t]*qs[t] for t in range(3)])
                knq = sum([on[t]*ps[t] for t in range(3)])
                ...
                newq = [0.0 if abs(x) < EPSILON else x for x in
                        [oq[t] - 2*kq*ps[t] for t in range(3)]] + [0]
                ...
                newnq = [0.0 if abs(x) < EPSILON else x for x in
                         [on[t] - 2*knq*ps[t] for t in range(3)]] + [0]
        ...
        if bar == 'a':
            ...
            for n in range(len(points)):
                edges += [(n,n+k+1) for k in range(len(points)-n-1)]
        return points, edges, colours
