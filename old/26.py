"""
Polyhedron Visualizer v0.26

This script displays the regular polyhedra, except the dodecahedron.
Views can now be selected from any axis.
"""
...
TITLE = "Polyhedron Visualizer v0.26"
...
class Canvas(tk.Canvas):
    ...
    def view(self, points):
        ...
        result = []
        while count < n:
            q = math.sqrt(points[count][0]**2+points[count][1]**2)
            a = math.atan2(points[count][1],points[count][0])-self._viewAxis[0]
            result.append((q*math.sin(a),
                           (points[count][2]/math.tan(self._viewAxis[1]) -
                            q*math.cos(a))*math.sin(self._viewAxis[1])))
            count += 1
        return result
