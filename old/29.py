...
TITLE = "Polyhedron Visualizer v0.29"
...
def norm(points):
    return math.sqrt(sum([x**2 for x in points]))
...
class Canvas(tk.Canvas):
    ...
    def view(self, points):
        ...
        if self._viewAxis[1] != 0:
            ...
            w = [n/norm(w) for n in w]
            h = [xa*za, ya*za, -ya*ya - xa*xa]
            h = [n/norm(h) for n in h]
        num = len(points)
        count = 0
        result = []
        while count < num:
    ...
    def rotate(self, rotAngle):
        ...
        while n < self._number:
            ...
            if op != (0,0,0):                
                ...
                oq = [q*norm(op)/norm(oq) for q in oq]
