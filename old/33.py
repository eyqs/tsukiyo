"""
Polychoron Visualizer v0.33

This script displays the regular polyhedra and the hypercube.
The 3D projection system now uses a retina analogy instead of a painter's.
"""
...
TITLE = "Polychoron Visualizer v0.33"
DESCRIPTION = "\nThis script displays the regular polyhedra and the hypercube."
pi = math.pi
WIDTH = 470
HEIGHT = 450
    ...
    def _view(self, points):
        ...
        while count < num:
            ...
            if self._viewAxis[1] == 0:
                if (self._viewAxis[0] == pi/2 or
                    self._viewAxis[0] == 3*pi/2):
                    n = (t*x-x)*st
                    m = (y-t*y)*st
                else:
                    n = ct*(y-t*y)-st*(x-t*x)
            ...
            result.append((n*self._radius, m*self._radius))
            count += 1
        return result
