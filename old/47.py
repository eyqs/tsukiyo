"""
Wythoff Visualizer v0.47

This program uses an iterative algorithm and keeps track of
rounded coordinates to ensure that there are no duplicates.
The resulting algorithm is faster than before but still
triplicate points, as there are three triangles at each point.
"""
...
TITLE = "Wythoff Visualizer v0.47"
...
class Canvas(tk.Canvas):
    ...
    def _wythoff(self, entry):
        ...
        coordinates = [''.join(sorted([numa, numb, numc]))]
        ...
        while depth > 0:
            for triangle in reversed(triangles):   # Reversed to avoid loop
                ...
                # Identify triangles with unique coordinate strings
                ...
                if numa not in coordinates:
                    triangles.append([newp, oq, os])
                    coordinates.append(numa)
                if numb not in coordinates:
                    triangles.append([op, newq, os])
                    coordinates.append(numb)
                if numc not in coordinates:
