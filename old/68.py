"""
Polyhedron Shader v0.68

This program shades convex polyhedra.
Planar polygons are now shaded as well, and nondegenerate stars are identified.
However, degenerate stars such as {4/2} and {6/3} are not labelled correctly,
and the density of the star is not calculated, but it would be easy to do so.
"""
...
def distance(head, tail):
    ...
    return sum([(head[i] - tail[i])**2 for i in range(4)])
    ...
    def set_status(self, event):
        ...
        if event == 'clear':
            ...
            name = {3:' triangle', 4:' square', 5:' pentagon', 6:' hexagon',
                    ...
                    11:' triangle', 12:' line', 13:' pentagram', 14:' hexagram',
                    15:' heptagram', 16:' octagram', 17:' nonagram', 18:' decagram'}
            text = ''
...
class Canvas(tk.Canvas):
    ...
    def get_data(self, event):
        ...
        if event == 'faces':
            return self._currPolytope.get_facesides()
    ...
    def _render(self):
        ...
        if self._currPolytope.get_points():
            ...
            if SHADING == True:    # Display polytope by drawing polygons
                ...
                camera = convert([self._distance*RADIUS] + self._viewAxis, True)
                self._currPolytope.shade_faces(camera)
                faces = self._currPolytope.get_faces()
                centres = self._currPolytope.get_centres()
            ...
            elif SHADING == False:  # Display polytope by drawing lines
                points = [(-point[0]+w, point[1]+h) for point in
                          self._view(self._currPolytope.get_points())]
...
class Polytope():
    ...
    def _set_faces(self, edges):
        ...
        self._faces = {}
        self._facesides = {i:0 for i in range(3,19)}    # 11 to 18 are stars
        self._visited = set()
        self._triangles = set()
    ...
    def _orientation(self, ab, bc):
        ...
        orientation = sum([cross(ab,bc)[i] for i in range(3)])
        if orientation < EPSILON:
            return 0
        return math.copysign(1, orientation)

    def _set_centres(self):
        """Create a list of centres using a list of faces and their points."""
        self._centres = {face:[0, 0, 0, 0] for face in self._faces}
        for face in self._faces:
            for point in self._faces[face]:
                for i in range(4):
                    self._centres[face][i] += self._points[point][i]
            for i in range(4):
                self._centres[face][i] /= len(self._faces[face])

    def _remove_faces(self):
        """Remove inside faces that are closer to the centre than normal."""
        ...
        surface = distance(self._centres[0], [0,0,0,0]) - depth
        for i in range(len(self._faces))[::-1]:     # pop from the end
            if distance(self._centres[i], [0,0,0,0]) < surface:
                self._facesides[len(self._faces[i])] -= 1
                self._faces.pop(i)
                self._centres.pop(i)

    def _set_colours(self):
        """Create a list of random colours applied to each face."""
        self._base_colours = {}
        for face in self._faces:
            r = 255
            g = 0
            b = 0
            self._base_colours[face] = [r,g,b]
            self._colours[face] = [r,g,b]

    def shade_faces(self, viewAxis):
        """Shade according to the angle between the light ray and the normal."""
        if len(self._faces) == 1:       # Shade both faces if polygon
            light = [viewAxis[i] - self._centres[0][i] for i in range(3)]
            ...
            normal = cross(u, v)        # self._centres[0] = [0,0,0,0]
            denominator = math.sqrt(abs(sum([x**2 for x in normal])
                                        * sum([x**2 for x in light])))
            cos = sum([light[i]*normal[i]/denominator for i in range(3)])
            for i in range(3):  # Lambert's cosine law for diffuse reflectors
                shade = self._base_colours[0][i] # Positive, but not brighter
                self._colours[0][i] = abs(shade * cos)
        else:
            for face in self._faces:
                light = [viewAxis[i] - self._centres[face][i] for i in range(3)]
                normal = self._centres[face]
                denominator = math.sqrt(abs(sum([x**2 for x in normal])
                                            * sum([x**2 for x in light])))
                cos = sum([light[i]*normal[i]/denominator for i in range(3)])
                for i in range(3):  # Lambert's cosine law for diffuse reflectors
                    shade = self._base_colours[face][i] # Positive, not brighter
                    self._colours[face][i] = max(0, shade * cos)
    ...
    def get_centres(self):
        """Return a list of centres of the faces of the polytope."""
    ...
    def get_facesides(self):
        """Return a list of numbers of faces with some amount of sides."""
