"""
Polyhedron Visualizer v0.24

This script displays the regular polyhedra, except the dodecahedron.
These regular polyhedra can now be rotated about any axis.
"""
...
TITLE = "Polyhedron Visualizer v0.24"
...
class Main(ttk.Frame):
    ...
    def _initUI(self):
        ...
        xViewBtn = ttk.Button(guiFrame, text="x", width=2,
                              command=lambda: self.canvas.new_view('x'))
        xViewBtn.bind("<Key-Return>", lambda event: self.canvas.new_view('x'))
        xViewBtn.grid(row=2, column=1)
        yViewBtn = ttk.Button(guiFrame, text="y", width=2,
                              command=lambda: self.canvas.new_view('y'))
        yViewBtn.bind("<Key-Return>", lambda event: self.canvas.new_view('y'))
        yViewBtn.grid(row=2, column=2)
        zViewBtn = ttk.Button(guiFrame, text="z", width=2,
                              command=lambda: self.canvas.new_view('z'))
        zViewBtn.bind("<Key-Return>", lambda event: self.canvas.new_view('z'))
