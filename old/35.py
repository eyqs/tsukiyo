"""
Polychoron Visualizer v0.35

This script displays the regular polyhedra and the hypercube.
The hypercube is displayed with a simple (x,y,z,w) -> (x,w) formula.
"""
...
TITLE = "Polychoron Visualizer v0.35"
DESCRIPTION = "\nThis script displays the regular polyhedra and the hypercube."
pi = math.pi
WIDTH = 500
HEIGHT = 450
...
class Canvas(tk.Canvas):
    ...
    def _view(self, points):
        ...
        if self._viewAxis[2] == 0:
            num = len(points)
            count = 0
            result = []
            while count < num:
                result.append((points[count][0], points[count][3]))
                count += 1
            return result
