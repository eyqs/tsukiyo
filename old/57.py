"""
Polyhedron Shader v0.57

This program shades simple polygons monochromatically.
The constant SHADING determines whether polytopes are rendered
as polygonal faces with borders or as coloured vertices and edges.
_schlafli2D() now only outputs faces, which cannot rotate.
"""
...
TITLE = 'Polyhedron Shader v0.57'
DESCRIPTION = '\nThis script displays shaded polygons.'
SHADING = True
pi = math.pi
WIDTH = 600
HEIGHT = 550
...
class Canvas(tk.Canvas):
    ...
    def _schlafli2D(self, entry):
        # Take 2D Schlafli symbol and return its one face.
        num = entry.split('/')
        p = int(num[0])
        d = 1
        if len(num) > 1:
    ...
    def _render(self):
        ...
        if SHADING == True:    # Display polytope by drawing polygons
            if self._currPolytope.get_faces():
                for face in self._currPolytope.get_faces():
                    self.create_polygon([(-point[0]+w, point[1]+h)
                                         for point in self._view(face)],
                                        fill='#F00', outline='#000', width=3)
...
class Polytope():
    ...
    def __init__(self, data):
        ...
        if data:
            if len(data) > 1:
                if len(data[1][0]) == 2:    # 2 floats for edge points
                    self._points = data[0]
                    self._edges = data[1]
                    self._colours = data[2]
                    self._faces = []
                elif len(data[1][0]) == 4:  # 4 floats for 4D coordinates
                    self._points = []
                    self._edges = []
                    self._colours = []
                    self._faces = data
