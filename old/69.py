"""
Polyhedron Shader v0.69

This program shades convex polyhedra.
Point colours and face colours are now separated inside the polytope object.
zoom and distance values are now managed in Main, which automatically
switches to wireframes for star polytopes and when distance < 200.
"""
...
class Main(ttk.Frame):
    ...
    def _initUI(self):
        ...
        self.wire = tk.IntVar()
        self._wireCheck = ttk.Checkbutton(_guiBottom, text='Wireframe',
                                         variable=self.wire, command=lambda:
                                         self.change('w'))
        self._wireCheck.grid(row=3, column=0, sticky=tk.W)
        ...
        self.distance.set(DISTANCE*RADIUS)
    ...
    def change(self, change, amount=CHANGE):
        ...
        if change == 's':
            ...
            self.zoom.set(self.zoom.get() + amount)
        elif change == 'zDown':
            self.zoom.set(self.zoom.get() - amount)
        elif change == 'dUp':
            self.distance.set(self.distance.get() + amount*0.1*RADIUS)
        elif change == 'dDown':
            self.distance.set(self.distance.get() - amount*0.1*RADIUS)
            if abs(self.distance.get()) < 200:
                self.change('wire')
        elif change == 'reset':
...
class Canvas(tk.Canvas):
    ...
    def get_data(self, event):
        ...
        if event == 'faces':
            facesides = self._currPolytope.get_facesides()
            for i in range(11,19):
                if facesides[i] > 0 and self.parent.wire.get() == 0:
...
class Polytope():
    ...
    def _remove_faces(self):
        ...
        surface = distance(self._centres[0]) - depth
        for i in range(len(self._faces))[::-1]:     # pop from the end
            if distance(self._centres[i]) < surface:
    ...
    def shade_faces(self, viewAxis):
        ...
        if len(self._faces) == 1:   # Shade both faces if polygon by using abs()
            ...
            for i in range(3):
                shade = self._base_colours[0][i]
                self._face_colours[0][i] = abs(shade * cosine)
        else:
            for face in self._faces:
                ...
                for i in range(3):  # Lambert's cosine law for diffuse reflectors
                    ...
                    self._face_colours[face][i] = max(0, shade * cosine)
