"""
Wythoff Visualizer v0.49

This program reflects a point that bisects
an edge of the fundamental triangle region
to all the triangles that cover the sphere.
Edges now connect these points, but the
algorithm is faulty since the side lengths
are often unequal. It is not the edge that
must be bisected, but the opposite angle.
"""
...
TITLE = "Wythoff Visualizer v0.49"
...
class Canvas(tk.Canvas):
    ...
    def _wythoff(self, entry):
        ...
        # p = p|qs, q = q|ps, s = s|pq
        # pq = pq|s, qs = qs|p, sp = sp|q
        # c = pqs|, d = |pqs, a = all triangles
        bar = 'pq'
        ...
        # Fundamental triangle without negative zeros
        ...
        if bar == 'p':
            ...
            numn = normalize([(triangles[0][0][t] + triangles[0][1][t])/2
                               for t in range(4)], [r])
        elif bar == 'qs':
            numn = normalize([(triangles[0][1][t] + triangles[0][2][t])/2
                               for t in range(4)], [r])
        elif bar == 'sp':
            numn = normalize([(triangles[0][2][t] + triangles[0][0][t])/2
                               for t in range(4)], [r])
        triangles[0].append(numn)
        ...
        tricoords = [''.join(sorted([numa, numb, numc]))]
        pointcoords = [numn]
        ...
        while depth > 0:
            for triangle in reversed(triangles):   # Reversed to avoid loop
                ...
                qs = normalize((oq[1]*os[2]-oq[2]*os[1],
                      oq[2]*os[0]-oq[0]*os[2],
                      oq[0]*os[1]-oq[1]*os[0]))
                sp = normalize((os[1]*op[2]-os[2]*op[1],
                      os[2]*op[0]-os[0]*op[2],
                      os[0]*op[1]-os[1]*op[0]))
                ...
                # Find the side length the first time
                if side == 0:
                    if bar == 'p':
                        side = math.sqrt(sum([(newp[t]-op[t])**2 for t in range(3)]))
                    elif bar == 'q':
                        side = math.sqrt(sum([(newq[t]-oq[t])**2 for t in range(3)]))
                    elif bar == 's':
                        side = math.sqrt(sum([(news[t]-os[t])**2 for t in range(3)]))
                    elif bar == 'pq':
                        side = math.sqrt(sum([(newnp[t]-on[t])**2 for t in range(3)]))
                    elif bar == 'qs':
                        side = math.sqrt(sum([(newnq[t]-on[t])**2 for t in range(3)]))
                    elif bar == 'sp':
                        side = math.sqrt(sum([(newns[t]-on[t])**2 for t in range(3)]))
                    elif bar == 'c':
                        side = 1
                    elif bar == 'd':
                        side = 1
                ...
                numa = ''.join(sorted([nuwp, numq, nums]))
                numb = ''.join(sorted([nump, nuwq, nums]))
                numc = ''.join(sorted([nump, numq, nuws]))
        ...
        if bar == 'a':
            ...
            if bar == 'p':
                colours += [(n, '#F00') for n in range(len(points))]
            elif bar == 'q':
                colours += [(n, '#000') for n in range(len(points))]
            elif bar == 's':
                colours += [(n, '#00F') for n in range(len(points))]
            for n in range(len(points)):
                for k in range(n+1, len(points)):
                    if math.sqrt(sum([(points[n][t]-points[k][t])**2
