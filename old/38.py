"""
Polychoron Visualizer v0.38

This script displays the regular polyhedra and the hypercube.
The hypercube can now rotate about any of the six standard axis-planes.
"""
...
TITLE = "Polychoron Visualizer v0.38"
DESCRIPTION = "\nThis script displays the regular polyhedra and the hypercube."
pi = math.pi
WIDTH = 500
HEIGHT = 450
...
DISTANCE = 50
RETINA = 10
CHANGE = 10    # 10 pixels change in radius and 0.01r change in distance
DELAY = 1    # 28 ms per pi/12 rotation = 3 rotations every 2 seconds
ROTANGLE = pi/100
EPSILON = 0.00001
GODMODE = False
DATA = []
...
class Canvas(tk.Canvas):
    ...
    def _translate(self, entry):
        ...
        # Hyperspherical coordinates of a hypercube, from 14/10/20
        elif entry == "h" or entry == "t":
            return Polytope([[RADIUS]*16,[pi/4]*4+[3*pi/4]*4+[5*pi/4]*4+[7*pi/4]*4,
             [0.9553166, 0.9553166, 2.186276, 2.186276]*4,[pi/3, 2*pi/3]*8,
             [(0,1),(0,2),(0,12),(0,4),(1,3),(1,5),(1,13),(2,3),(2,6),
              (2,14),(3,7),(3,15),(4,5),(4,6),(4,8),(5,7),(5,9),(6,10),
              (6,7),(7,11),(8,9),(8,10),(8,12),(9,11),(9,13),(10,11),
              (10,14),(12,13),(12,14),(13,15),(11,15),(14,15)]])
...
class Polytope():
    ...
    def get_points(self):
        """
        Convert hyperspherical coordinates (four separate lists)
        to return Cartesian coordinates (one list of quadruples).
        """
        points = []
        n = 0
        while n < self._number:
            points.append(
                (self._rs[n]*math.sin(self._omegas[n])*math.sin(self._phis[n])*math.cos(self._thetas[n]),
                 self._rs[n]*math.sin(self._omegas[n])*math.sin(self._phis[n])*math.sin(self._thetas[n]),
                 self._rs[n]*math.sin(self._omegas[n])*math.cos(self._phis[n]),
                 self._rs[n]*math.cos(self._omegas[n])))
            n += 1
        return points
    ...
    def set_rotaxis(self, i, j):
        if i[0]*i[0]+i[1]*i[1]+i[2]*i[2] > 0 and j[0]*j[0]+j[1]*j[1]+j[2]*j[2] > 0:
            k = (i[1]*j[2]-i[2]*j[1],
                 i[2]*j[0]-i[0]*j[2],
                 i[0]*j[1]-i[1]*j[0],0)
        elif i[0]*i[0]+i[1]*i[1]+i[3]*i[3] > 0 and j[0]*j[0]+j[1]*j[1]+j[3]*j[3] > 0:
            k = (i[1]*j[3]-i[3]*j[1],
                 i[3]*j[0]-i[0]*j[3],
                 0,i[0]*j[1]-i[1]*j[0])
        elif i[0]*i[0]+i[2]*i[2]+i[3]*i[3] > 0 and j[0]*j[0]+j[2]*j[2]+j[3]*j[3] > 0:
            k = (i[2]*j[3]-i[3]*j[2],0,
                 i[3]*j[0]-i[0]*j[3],
                 i[0]*j[2]-i[2]*j[0])
        elif i[1]*i[1]+i[2]*i[2]*i[3]*i[3] > 0 and j[1]*j[1]+j[2]*j[2]+j[3]*j[3] > 0:
            k = (0,i[2]*j[3]-i[3]*j[2],
                 i[3]*j[1]-i[1]*j[3],
                 i[1]*j[2]-i[2]*j[1])
        else:
            raise ValueError
        l = (+i[1]*(j[2]*k[3]-k[2]*j[3])-i[2]*(j[1]*k[3]-k[1]*j[3])+i[3]*(j[1]*k[2]-k[1]*j[2]),
             -i[0]*(j[2]*k[3]-k[2]*j[3])+i[2]*(j[0]*k[3]-k[0]*j[3])-i[3]*(j[0]*k[2]-k[0]*j[2]),
             +i[0]*(j[1]*k[3]-k[1]*j[3])-i[1]*(j[0]*k[3]-k[0]*j[3])+i[3]*(j[0]*k[1]-k[0]*j[1]),
             -i[0]*(j[1]*k[2]-k[1]*j[2])+i[1]*(j[0]*k[2]-k[0]*j[2])-i[2]*(j[0]*k[1]-k[0]*j[1]))
        self._axis_i = i
        self._axis_j = j
        self._axis_k = k
        self._axis_l = l
    ...
    def rotate(self, rotAngle):
        ...
        n = 0
        while n < self._number:
            p = (self._rs[n]*math.sin(self._omegas[n])*math.sin(self._phis[n])*math.cos(self._thetas[n]),
                 self._rs[n]*math.sin(self._omegas[n])*math.sin(self._phis[n])*math.sin(self._thetas[n]),
                 self._rs[n]*math.sin(self._omegas[n])*math.cos(self._phis[n]),
                 self._rs[n]*math.cos(self._omegas[n]))
            r = sum([p[n]*i[n] for n in range(4)])
            s = sum([p[n]*j[n] for n in range(4)])
            I = [i[n]*r+j[n]*s for n in range(4)]
            ip = [p[n]-I[n] for n in range(4)]
            ...
            iq = normalize(iq, ip)
            rotP = [ip[t]*cos + iq[t]*sin + I[t] for t in range(4)]

            if rotP[0] >= 0:
                self._thetas[n] = math.acos(rotP[1]/math.sqrt(rotP[1]**2+rotP[0]**2))
            elif rotP[0] <= 0:
                self._thetas[n] = 2*pi-math.acos(rotP[1]/math.sqrt(rotP[1]**2+rotP[0]**2))
            self._phis[n] = abs(math.acos(rotP[2]/math.sqrt(rotP[2]**2+rotP[1]**2+rotP[0]**2)))
            self._omegas[n] = abs(math.acos(rotP[3]/self._rs[n]))
            n += 1
    ...
    def _check_restrictions(self):
        ...
        n = 0
        while n < self._number:
            while self._omegas[n] >= 2*pi:
                self._omegas[n] -= 2*pi
            while self._omegas[n] < 0:
                self._omegas[n] += 2*pi
            while self._phis[n] >= 2*pi:
                self._phis[n] -= 2*pi
            while self._phis[n] < 0:
                self._phis[n] += 2*pi
            while self._thetas[n] >= 2*pi:
                self._thetas[n] -= 2*pi
            while self._thetas[n] < 0:
                self._thetas[n] += 2*pi
            if self._omegas[n] > pi:
