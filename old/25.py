"""
Polyhedron Visualizer v0.25

This script displays the regular polyhedra, except the dodecahedron.
Views can now be selected from any axis on the xy, xz, and yz planes.
"""
...
TITLE = "Polyhedron Visualizer v0.25"
...
class Canvas(tk.Canvas):
    ...
    def new_view(self, viewAxis):
        ...
        #Set phi up so that the xy-plane is at phi = 0
        if viewAxis[1] <= math.pi/2:
            viewAxis[1] = math.pi/2 - viewAxis[1]
        elif (viewAxis[1] > math.pi/2 and
              viewAxis[1] < 3*math.pi/2):
            viewAxis[1] = math.pi/2 + viewAxis[1]
        elif viewAxis[1] >= 3*math.pi/2:
            viewAxis[1] = 5*math.pi/2 - viewAxis[1]

        if abs(viewAxis[1]) > 0.1 and viewAxis[0] % (math.pi/2) > 0.1:
    ...
    def view(self, points):
        ...
        # Formula derived on 14/09/26
        if abs(self._viewAxis[1]) < 0.1:
            return [(point[1]*math.cos(self._viewAxis[0]) -
                     point[0]*math.sin(self._viewAxis[0]),
                     point[2]) for point in points]
        if self._viewAxis[0] % math.pi < 0.1:
            return [(point[1], point[2]*math.cos(self._viewAxis[1]) -
                     point[0]*math.sin(self._viewAxis[1])) for point in points]
        if (self._viewAxis[0] - math.pi/2) % math.pi < 0.1:
            return [(point[0], point[2]*math.cos(self._viewAxis[1]) -
                     point[1]*math.sin(self._viewAxis[1])) for point in points]
        else:
            raise ValueError
