"""
Wythoff Visualizer v0.51

This program displays polyhedra based on their Wythoff symbol.
(pq|s) symbols now work, but not (pqs|) nor (|pqs).
Edges are now linked by finding a minimum length and then
connecting all points at the same distance with brute force.
"""
...
TITLE = 'Wythoff Visualizer v0.51'
    ...
    def _wythoff(self, entry='', selection='n', colour='#000'):
        ...
        if selection == 'n':
            if bar == 1:
                ...
                op = normalize(triangles[0][0])
                oq = triangles[0][1]
                pq = (op[1]*oq[2]-op[2]*oq[1],
                      op[2]*oq[0]-op[0]*oq[2],
                      op[0]*oq[1]-op[1]*oq[0])
                ot = normalize((pq[1]*op[2]-pq[2]*op[1],
                      pq[2]*op[0]-pq[0]*op[2],
                      pq[0]*op[1]-pq[1]*op[0]))
                n = [r*op[t]*math.cos(lpn) + r*ot[t]*math.sin(lpn)
            ...
            if selection == 'p':
                ...
                ot = normalize((pq[1]*op[2]-pq[2]*op[1],
                      pq[2]*op[0]-pq[0]*op[2],
                      pq[0]*op[1]-pq[1]*op[0]))
                n = [r*op[t]*math.cos(pn) + r*ot[t]*math.sin(pn)
                     for t in range(3)] + [0.0]
            elif selection == 'qs':
                bar = 2
                ...
                oq = normalize(triangles[0][1])
                os = triangles[0][2]
                qs = (oq[1]*os[2]-oq[2]*os[1],
                      oq[2]*os[0]-oq[0]*os[2],
                      oq[0]*os[1]-oq[1]*os[0])
                ot = normalize((qs[1]*oq[2]-qs[2]*oq[1],
                      qs[2]*oq[0]-qs[0]*oq[2],
                      qs[0]*oq[1]-qs[1]*oq[0]))
                n = [r*oq[t]*math.cos(qn) + r*ot[t]*math.sin(qn)
                     for t in range(3)] + [0.0]
            elif selection == 'sp':
                bar = 2
                ...
                os = normalize(triangles[0][2])
                op = triangles[0][0]
                sp = (os[1]*op[2]-os[2]*op[1],
                      os[2]*op[0]-os[0]*op[2],
                      os[0]*op[1]-os[1]*op[0])
                ot = normalize((sp[1]*os[2]-sp[2]*os[1],
                      sp[2]*os[0]-sp[0]*os[2],
                      sp[0]*os[1]-sp[1]*os[0]))
                n = [r*os[t]*math.cos(sn) + r*ot[t]*math.sin(sn)
                     for t in range(3)] + [0.0]
        ...
        for point in points[1:]:
            if sum([(point[t] - points[0][t])**2 for t in range(3)]) < side:
                side = sum([(point[t] - points[0][t])**2 for t in range(3)])
