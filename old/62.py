"""
Polyhedron Shader v0.62

This program shades polyhedra without regards to camera position.
The breadth-first search keeps track of two consecutive edges already in
discovered faces, so the duplicates are quite reduced for large side numbers:
{5.3} 720 to 72; (2 3 5 | ) 4320 to 537; but (2 3 5) 5760 to still 2400.
"""
...
class Polytope():
    ...
    def _bfs(self, end, start, length):
        ...
        # Run the loop explicitly for triangles when fewer than two edges
        frontier = [neighbour for neighbour in self._graph[start]]
        paths = [[start] + [neighbour] for neighbour in self._graph[start]]
        depthTime = len(self._graph[start])
        while depthTime > 0:
            vertex = frontier.pop()
            path = paths.pop()
            if length == 3:
        ...
        while depth <= length:
            ...
            two = sorted([[pathEnd] + [pathEnd[::-1]]])[0][0]
            if two not in self._visited:
                self._visited.append(two)
                if depth == length:
                    for neighbour in self._graph[vertex]:
                        if neighbour == end:
                            path.append(end)
