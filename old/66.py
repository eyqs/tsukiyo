"""
Polyhedron Shader v0.66

This program shades convex polyhedra.
All the simple Wythoff polyhedra are divided into two classes based on
if faces closer to the centre than normal should be kept or removed.
The status now displays the number of each polygon type in each polyhedra.
"""
        ...
        if self._currPolytope.get_points():
            ...
            if SHADING == True:    # Display polytope by drawing polygons
                ...
                for i in range(len(centres)):
                    distances[i] = distance(centres[i],
                     convert([self._distance*RADIUS] + self._viewAxis, True))
...
class Polytope():
            ...
            while j < len(self._points):
                for start in self._graph[j]:
                    ...
                    if faces:       # At most two faces can go from start to j
                        self._faces = self._faces + [tuple(face) for
                                                     face in faces]
    ...
    def _set_centres(self):
        ...
        self._centres = [[0, 0, 0, 0] for face in self._faces]
        for i in range(len(self._faces)):
            for point in self._faces[i]:
                for j in range(4):
                    self._centres[i][j] += self._points[point][j]
            for j in range(4):
                self._centres[i][j] = self._centres[i][j] / len(self._faces[i])

    def _remove_faces(self):
        """Remove inside faces that are closer to the centre than normal."""
        if len(self.C_faces) < 32 and self._facesides[3]/len(self._faces) < 0.6:
