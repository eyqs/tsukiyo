"""
Polyhedron Shader v0.59

This program shades simple triangles monochromatically.
Graphs now replace both edges and faces by storing vertex adjacency data.
_set_faces extrapolates face data using only existing edge data.
Currently only works for triangles, including the regular deltahedra.
"""
...
TITLE = 'Polyhedron Shader v0.59'
DESCRIPTION = '\nThis script displays shaded triangles.'
SHADING = True
pi = math.pi
WIDTH = 600
HEIGHT = 550
...
class Polytope():
    ...
    def _set_faces(self, edges):
        ...
        # Only triangles work for now
        i = 0
        while i < self._edges.getSize():
            vertex = self._edges.getVertex(i)
            neighbours = vertex.getNeighbours()
            j = 0
            while j < len(neighbours):
                neighbors = neighbours[j].getNeighbours()
                if set(neighbours) & set(neighbors):
                    n = neighbours[j].getNumber()
                    self._faces += [(i,n,k.getNumber()) for k in
                                    set(neighbours) & set(neighbors)]
                j += 1
            i += 1
