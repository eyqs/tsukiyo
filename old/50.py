"""
Wythoff Visualizer v0.50

This program displays polyhedra based on their Wythoff symbol.
The Wythoff symbols are neither correctly parsed nor processed.
The GUI is now sectioned into four separate frames.
"""
...
TITLE = 'Wythoff Visualizer v0.50'
...
class Canvas(tk.Canvas):
    ...
    def set_bar(self, bar):
        ...
        [p,q,s] = self._currWythoff
        if bar == 'a':
            self.currPolytope = Polytope(self._wythoff(' '.join((p,q,s))))
        elif bar == 'b':
            self.currPolytope = Polytope(self._wythoff(' '.join((p,q,s,'|'))))
        elif bar == 'c':
            self.currPolytope = Polytope(self._wythoff(' '.join(('|',p,q,s))))
        elif bar == 'p':
            self.currPolytope = Polytope(self._wythoff(' '.join((p,'|',q,s)),'#f00'))
        elif bar == 'q':
            self.currPolytope = Polytope(self._wythoff(' '.join((q,'|',s,p)),'#000'))
        elif bar == 's':
            self.currPolytope = Polytope(self._wythoff(' '.join((s,'|',p,q)),'#00F'))
        elif bar == 'pq':
            self.currPolytope = Polytope(self._wythoff(' '.join((p,q,'|',s))))
        elif bar == 'qs':
            self.currPolytope = Polytope(self._wythoff(' '.join((q,s,'|',p))))
        elif bar == 'sp':
            self.currPolytope = Polytope(self._wythoff(' '.join((s,p,'|',q))))
        self._reset()
        self._render()
    ...
    def _wythoff(self, entry, colour='#000'):
        ...
        qPhi = math.acos((math.cos(pi/s) + math.cos(pi/p)*math.cos(pi/q))/
                         (math.sin(pi/p)*math.sin(pi/q)))
        sPhi = math.acos((math.cos(pi/q) + math.cos(pi/p)*math.cos(pi/s))/
                         (math.sin(pi/p)*math.sin(pi/s)))
