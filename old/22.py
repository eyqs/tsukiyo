"""
Polyhedron Visualizer v0.22

This script displays the regular polyhedra.
This update attempts to rectify rotation capabilities.
"""
...
TITLE = "Polyhedron Visualizer v0.22"
DESCRIPTION = "\nThis script displays the regular polyhedra."
WIDTH = 400
HEIGHT = 400
RADIUS = 100
...
class Main(ttk.Frame):
    ...
    def _initUI(self):
        ...
        thetaDownBtn = ttk.Button(guiFrame, image=self._downBtn, state=tk.DISABLED,
        ...
        phiDownBtn = ttk.Button(guiFrame, image=self._downBtn, state=tk.DISABLED,
...
class Canvas(tk.Canvas):
    ...
    def inject_data(self):
        ...
        self.currPolytope = Polytope(self._schlafli3D(DATA))
        self.angle = 0
        self._temporary_rotate()

    def _temporary_rotate(self):
        self.angle += ROTANGLE
        self.delete(tk.ALL)
        ...
        points = [(point[0]+w, point[1]+h) for point in
                  self.view(self.currPolytope.rotate_x(self.angle))]
        ...
        for edge in edges:
            ...
            self.create_line(points[edge[0]], points[edge[1]],
                             fill='#CCC', width=5)
        self.after(100, self._temporary_rotate)
    ...
    def _schlafli3D(self, entry):
        ...
        if round(firstPhi,4) <= round(math.pi/2,4):    # Rounding for float errors
            ...
            if p == 5:
                # Coordinates of the top middle dodecahedron ring
                a = math.asin(math.sqrt(2*(math.cos(ap)-math.cos(aq))*
                                        (1-math.cos(ap))/(1-math.cos(aq))))
                b = math.acos(2*(math.cos(ap)-math.cos(aq))/(1-math.cos(aq)))
                rs += [r]*2*q
                thetas += [k*aq+a for k in range(q)]
                thetas += [k*aq+a+b for k in range(q)]
                secondPhi = 2*math.atan(math.sqrt(
                    (1+2*math.cos(ap)-3*math.cos(aq))/(2*(1-math.cos(aq)))))
                phis += [secondPhi]*2*q

                # Coordinates of the bottom middle dodecahedron ring
                rs += [r]*2*q
...
class Polytope():
    ...
    def rotate_x(self, rotAngle):
        ...
        while n < self._number:
            ...
            axis = (1,0,0)
            o = (self._rs[n]*math.sin(self._phis[n])
                 *math.cos(self._thetas[n]),0,0)
            ...
            plane = axis[0]*p[0] + axis[1]*p[1] + axis[2]*p[2]
            ...
            c = math.cos(rotAngle)
            s = math.sin(rotAngle)
            rotP = (op[0]*c+oq[0]*s, op[1]*c+oq[1]*s, op[2]*c+oq[2]*s)
            points.append(rotP)
            n += 1
        return(points)
