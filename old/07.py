"""
Polygon Visualizer v0.07

This script can parse all 2D
Schlafli symbols and centres
...
class Main(tk.Frame):
    ...
    def polarToCartesian(self, number, rs, thetas): #also correctly centres points so that (0,0) is at centre of screen
        points = []
        count = 0
        while count < number:                                   #(r,t)->(rcost, rsint), self.parent.winfo_width()//2 is centre
            points.append((rs[count]*math.cos(thetas[count])+self.parent.winfo_width() // 2,
                           rs[count]*math.sin(thetas[count])+self.parent.winfo_height() // 2))
            count += 1
        return points
