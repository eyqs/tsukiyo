"""
Polyhedron Visualizer v0.18

This script displays the top circle of vertices of regular polyhedra.
"""
...
TITLE = "Polyhedron Visualizer v0.18"
...
class Canvas(tk.Canvas):
    ...
    def _schlafli3D(self, entry):
        ...
        r = RADIUS
        rs = [r]*(q+1)
        thetas = [0] + [(2*k*math.pi/q) for k in range(q)]
        phis = [0] + [2*math.acos(math.sqrt((1-math.cos(math.pi*(1-2/p)))/
                                            (1-math.cos(2*math.pi/q))))]*q
        return rs, thetas, phis
