"""
Polyhedron Shader v0.60

This program shades simple polyhedra monochromatically.
_set_faces now uses a simple breadth-first search to find cycles.
However, it is very inefficient and duplicate faces are not removed.
Some polygons and regular polyhedra are correct but never the dodecahedron.
"""
...
TITLE = 'Polyhedron Shader v0.60'
DESCRIPTION = '\nThis script displays shaded simple polyhedra.'
SHADING = True
pi = math.pi
WIDTH = 600
HEIGHT = 550
        ...
        if SHADING == True:    # Display polytope by drawing polygons
            if self._currPolytope.get_faces():
                ...
                for face in faces:
                    ...
                    self.create_polygon(edges, fill='#F00', outline='#000', width=3)
...
class Vertex:
    """
    Data class that stores adjacency data for each individual vertex.
...
class Graph:
    """
    Data class that stores graph data for all edges and faces.
    ...
    def getSize(self):
        return self._size
    def _addVertex(self,num):
        vertex = Vertex(num)
        self._vertices[num] = vertex
        self._size += 1
    def getVertex(self,num):
        if num in self._vertices:
            return self._vertices[num]
        else:
            return None
    def addEdge(self,start,end):
        if start not in self._vertices:
            self._addVertex(start)
        if end not in self._vertices:
            self._addVertex(end)
        self._vertices[start].addNeighbour(self._vertices[end])
        self._vertices[end].addNeighbour(self._vertices[start])
...
class Polytope():
    ...
    def _set_faces(self, edges):
        ...
        self._edges = Graph()
        for edge in edges:
            self._edges.addEdge(edge[0], edge[1])
        self._faces = []

        i = 3           # Iterate across all polygon side numbers
        while i < 4:
            j = 0       # Iterate across all vertices in graph
            while j < self._edges.getSize():
                start = self._edges.getVertex(j)        # Only two faces can go
                for vertex in start.getNeighbours():    # from vertex to start
                    for neighbour in vertex.getNeighbours():
                        face = self._bfs(start, vertex, i)
                        if face:
    ...
    def _bfs(self, end, start, length):
        ...
        frontier = [start]
        paths = [[start.getNumber()]]
        depth = 2                               # Depth of next layer, short as
        depthTime = len(start.getNeighbours())  # start already went to vertex
        while depth <= length:
            vertex = frontier.pop()
            path = paths.pop()
            depthTime -= 1
            if depth == length:
                for neighbour in vertex.getNeighbours():
                    if neighbour.getNumber() == end.getNumber():
                        path.append(end.getNumber())
                        return path
            else:
                frontier = [neighbour for neighbour
                            in vertex.getNeighbours()] + frontier
                paths = [path + [neighbour.getNumber()] for neighbour
                         in vertex.getNeighbours()] + paths
            if depthTime == 0:
                depth += 1
