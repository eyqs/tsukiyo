"""
Polyhedron Shader v0.70

This program shades convex polyhedra.
Wireframe edges are now generally displayed correctly after sorting by distance,
with correspondingly varied shades and widths, except when viewed from inside.
Zooming now follows a x^(-3/2) distribution, which slows it down when close.
"""
    ...
    def change(self, change):
        ...
        if change == 's':
            ...
            if self.unitDistance < 1:
                self.unitDistance = 1
            self.distance.set(int(ZOOM*RADIUS*RETINA/(self.unitDistance**(3/2))))
            if self.unitDistance < ZOOM*RETINA:
...
class Canvas(tk.Canvas):
    ...
    def get_data(self, event):
        ...
        if event == 'faces':
            faceSides = self._currPolytope.get_face_sides()
