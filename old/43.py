"""
Wythoff Visualizer v0.43

This script displays the double pyramids.
I tried to extend these to all Wythoff symbols but I failed.
"""
...
TITLE = "Wythoff Visualizer v0.43"
...
class Canvas(tk.Canvas):
    ...
    def _wythoff(self, entry):
        ...
        if p > 2 or q > 3 or (q == 3 and s > 5):
            raise ValueError
        firstPhi = math.acos(s-s/p-s/q)

        # Triangles of the first ring
        for k in range(s):
            triangles += [((0,0), (2*pi*k/s, firstPhi),
                           (2*pi*k/s + pi/s, firstPhi)),
                          ((0,0), (2*pi*k/s, firstPhi),
                           (2*pi*k/s - pi/s, firstPhi))]

        # Triangles of the last (2 2 s) ring
        if q == 2:
            for k in range(s):
                triangles += [((0,pi),(2*pi*k/s, pi-firstPhi),
                               (2*pi*k/s + pi/s, pi-firstPhi)),
                              ((0,pi),(2*pi*k/s, pi-firstPhi),
                               (2*pi*k/s - pi/s, pi-firstPhi))]
        elif q == 3:
            if s == 3:
                for k in range(s):
                    triangles += [    # Triangles of the last (2 3 3) ring
                        ((2*pi*k/s - pi/2/s, pi-firstPhi),(0,pi),
                         (2*pi*k/s + pi/2/s, pi-firstPhi)),
                        ((2*pi*k/s - pi/2/s, pi-firstPhi),(0,pi),
                         (2*pi*k/s - 3*pi/2/s, pi-firstPhi)),
                        # Triangles of the middle (2 3 3) ring
                        ((2*pi*k/s - pi/2/s, pi-firstPhi),
                         (2*pi*k/s, firstPhi), (2*pi*k/s - pi/s, firstPhi)),
                        ((2*pi*k/s - pi/2/s, pi-firstPhi),
                         (2*pi*k/s, firstPhi), (2*pi*k/s + pi/2/s, pi-firstPhi))]

            # Triangles of the last (2 3 s) ring
            else:
                for k in range(s):
                    triangles += [((0,pi),(2*pi*k/s + pi/2/s, pi-firstPhi),
                                   (2*pi*k/s + pi/s + pi/2/s, pi-firstPhi)),
                                  ((0,pi),(2*pi*k/s + pi/2/s, pi-firstPhi),
                                   (2*pi*k/s - pi/s + pi/2/s, pi-firstPhi))]

        n = 0
        for triangle in triangles:
            thetas += [p[0] for p in triangle]
            phis += [p[1] for p in triangle]
            edges += [(n,n+1), (n+1,n+2), (n,n+2)]
            colours += [(n, '#000'), (n+1, '#F00'), (n+2, '#00F')]
            n += 3
