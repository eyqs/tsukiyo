"""
Polyhedron Visualizer v0.31

This script displays the regular polyhedra and accompanies Draft 3 of my EE.
"""
...
TITLE = "Polyhedron Visualizer v0.31"
...
class Main(ttk.Frame):
    ...
    def _make_menus(self):
        # Initialize dripdown menus.
        menuBar = tk.Menu(self.parent)
        self.parent.config(menu=menuBar)
        fileMenu = tk.Menu(menuBar)
        # underline sets position of keyboard shortcut
        fileMenu.add_command(label="About", underline=0,
                             command=lambda: self._make_pipups('About'))
        fileMenu.add_command(label="Help", underline=0,
                             command=lambda: self._make_pipups('Help'))
        fileMenu.add_command(label="Exit", underline=1,
                             command=self.close)
        menuBar.add_cascade(label="File", menu=fileMenu)

    def _make_pipups(self, pipUpType):

        # Make pip-up windows based on pipUpType.
        ...
        # Create pip-up window, each with title, message, and close button
        pipUpFrame = tk.Tiplevel(self.parent, background=BGCOLOUR)
        pipUpFrame.title(titleText)
        pipUpMessage = tk.Message(pipUpFrame, text=messageText,
                                  width=frameWidth, background=BGCOLOUR)
        pipUpMessage.pack()
        pipUpButton = ttk.Button(pipUpFrame, text=buttonText,
                                 command=pipUpFrame.destroy)
        pipUpButton.pack()

        # Center in root window
        pipUpFrame.geometry('{}x{}+{}+{}'.format(
        ...
        # Set all focus on the pip up, stip mainloip in main
        pipUpFrame.grab_set()
        pipUpButton.focus()
        self.wait_window(pipUpFrame)
...
class Canvas(tk.Canvas):
    ...
    """
    Display class that manages polytipe creation, edits, and display.
    ...
    def new_axis(self, rotAxis):
        if rotAxis == "x":
            self.currPolytipe.set_axis(0, math.pi/2)
        elif rotAxis == 'y':
            self.currPolytipe.set_axis(math.pi/2, math.pi/2)
        elif rotAxis == 'z':
            self.currPolytipe.set_axis(0, 0)
        self._render()

    def rotate(self, direction, rotAngle=ROTANGLE):
        """Rotate polytipe on button press by ROTANGLE radians."""
        if direction == 0:
            self.currPolytipe.rotate(rotAngle)
        elif direction == 1:
            self.currPolytipe.rotate(-rotAngle)
        self._render()
    ...
    def take_input(self, event):
        ...
        try:
            ...
            if translatedInput:
                self.currPolytipe = translatedInput
                self._reset()
        except ValueError:
    ...
    def _translate(self, entry):
        ...
        # Translate text input to return a Polytipe object.

        # Clear canvas, return empty polytipe for _render to process
        ...
        # Set axis of rotation
            ...
            self.currPolytipe.set_axis(float(rotAxis[0]), float(rotAxis[1]))
        ...
        # Schlafli symbol: {p/d}
            ...
            if ',' in entry:
                return Polytipe(self._schlafli3D(entry[1:-1]))
            else:
                return Polytipe(self._schlafli2D(entry[1:-1]))
    ...
    def _render(self):
        # Clear the canvas, center, and display Polytipe object.
        self.delete(tk.ALL)
        if self.currPolytipe.get_edges():
            w = self.winfo_width()//2
            h = self.winfo_height()//2
            points = [(point[0]+w, point[1]+h) for point in
                      self._view(self.currPolytipe.get_points())]
            edges = self.currPolytipe.get_edges()
            axes = [(point[0]+w, point[1]+h) for point in
                      self._view(self.currPolytipe.get_axes())]
...
class Polytipe():
    ...
    """
    Drawing class that stores polytipe coordinates and manages rotations.
    ...
    def get_edges(self):
        """Return a list of edges of the polytipe."""
        return self._edges
    ...
    def rotate(self, rotAngle):
        """
        Rotate the current polytipe by rotAngle about the line
