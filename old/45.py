"""
Wythoff Visualizer v0.45

This script displays the tetrakis hexahedron.
All triangle vertices are now well-defined.
Each type of vertex can be selected separately to show some polyhedron.
"""
...
TITLE = "Wythoff Visualizer v0.45"
...
class Canvas(tk.Canvas):
    ...
    def _wythoff(self, entry):
        ...
        for k in range(p):
            triangles += [((2*pi*k/p, sPhi),
                           (2*pi*k/p - pi/p, qPhi),(0,0)),
                          ((2*pi*k/p, sPhi),
                           (2*pi*k/p + pi/p, qPhi),(0,0))]
        # Triangles of the last (p 2 2) ring
        if q == 2:
            for k in range(p):
                triangles += [((2*pi*k/p - pi/p, pi-sPhi),
                               (2*pi*k/p, pi-qPhi),(0,pi)),
                              ((2*pi*k/p + pi/p, pi-sPhi),
                               (2*pi*k/p, pi-qPhi),(0,pi))]
        elif q == 3:
            if p == 3:
                # Triangles of the last (3 3 2) ring
                for k in range(p):
                    triangles += [((2*pi*k/p - pi/p, pi-sPhi),
                                   (0,pi),(2*pi*k/p, pi-qPhi)),
                                  ((2*pi*k/p + pi/p, pi-sPhi),
                                   (0,pi),(2*pi*k/p, pi-qPhi))]
                # Triangles of the middle (3 3 2) ring
                for k in range(p):
                    triangles += [((2*pi*k/p - pi/p, pi-sPhi),
                                   (2*pi*k/p - pi/p, qPhi),
                                   (2*pi*k/p, pi-qPhi)),
        ...
        if num[3] == 'a':
            for triangle in triangles:
                thetas += [t[0] for t in triangle]
                phis += [t[1] for t in triangle]
                edges += [(n,n+1), (n+1,n+2), (n,n+2)]
                colours += [(n, '#F00'), (n+1, '#000'), (n+2, '#00F')]
                n += 3
        else:
            # Edges of the first ring
            edges += [(k,(k+1)%(2*p)) for k in range(2*p)]
            # Edges of the last ring
            edges += [(2*p+k,2*p+(k+1)%(2*p)) for k in range(2*p)]
            if q == 3:
                # Edges of the middle ring
                edges += [(4*p+k,4*p+(k+1)%(4*p)) for k in range(4*p)]
                if p == 3:
                    # Edges between the middle and first (3 3 2) rings
                    for k in range(2*p):
                        edges += [(k,4*p+(2*k+j-1)%(4*p)) for j in range(5)]
                        edges += [(2*p+k,4*p+(2*k+j-2)%(4*p)) for j in range(5)]
            if num[3] == 'b':
                for t in triangles:
                    thetas.append(t[0][0])
                    phis.append(t[0][1])
